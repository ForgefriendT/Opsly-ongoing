...
export async function handleSendEstimateEmail(req, res, preParsedBody) {
  try {
    ...
    const { estimateId, recipientEmail, recipientName, businessName } = body

    if (!estimateId || !recipientEmail) {
      res.writeHead(400, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({ error: 'Missing estimateId or recipientEmail' }))
      return
    }

    // 1. Fetch estimate and client details using admin client
    const { data: estimate, error: estimateErr } = await supabaseAdmin
      .from('estimates')
    ...
    if (estimateErr || !estimate) {
      console.error('Error fetching estimate for email:', estimateErr)
    ...
    const logoUrl = estimate.client?.logo_url
    const currencySym = estimate.currency_symbol || '$'
    const totalAmount = Number(estimate.grand_total || 0).toFixed(2)
    const validUntilStr = estimate.valid_until ? new Date(estimate.valid_until).toLocaleDateString() : '—'
    
    // Resolve dynamic app base URL from request headers
    const protocol = req.headers['x-forwarded-proto'] || 'http'
    const host = req.headers.host
    const appUrl = `${protocol}://${host}`
    const portalUrl = `${appUrl}/estimate/${estimate.id}`
    ...
    const htmlContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Estimate ${estimate.estimate_number} from ${businessName}</title>
        <style>
          body {
      ...
      <body>
        <div class="container">
          ...
          <h1>Estimate Proposal ${estimate.estimate_number}</h1>
          <p>Hello ${recipientName || 'Valued Customer'},</p>
          <p>${businessName} has sent you a new estimate proposal for <strong>${currencySym}${totalAmount}</strong>, valid until <strong>${validUntilStr}</strong>.</p>
          
          <div class="details-card">
            <div class="detail-row">
              <span class="label">Estimate Number</span>
              <span class="value">${estimate.estimate_number}</span>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
              <span class="label">Proposal Value</span>
              <span class="value highlight">${currencySym}${totalAmount}</span>
            </div>
          </div>

          <a href="${portalUrl}" class="btn" style="color: #f4f3ee;">Review & Approve Estimate</a>

          <p>Click the button above to open a secure page where you can review the line-item details, print or download the proposal, and approve or reject the terms online.</p>
    ...
    if (!resendApiKey) {
      console.warn('RESEND_API_KEY is not defined in env. Simulating estimate email dispatch.')
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [recipientEmail],
        subject: `Estimate Proposal ${estimate.estimate_number} from ${businessName}`,
        html: htmlContent
      })
    })
    ...
    if (!resendRes.ok) {
      ...
      console.error('Resend API Error (Estimates):', resendErr)
    ...
    console.log(`Email dispatched successfully for Estimate ${estimate.estimate_number} to ${recipientEmail}:`, resendData)
